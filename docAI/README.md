...
A desktop application for technical writers that provides content analysis, DITA XML conversion, impact analysis, and a lightweight document editor ΓÇö all in one integrated workspace.
...
- **Write with confidence** ΓÇö The Content Analysis engine checks your text against 60+ writing rules in real time, catching issues like passive voice, vague modifiers, biased terminology, and non-standard word usage before review cycles begin.

- **Convert faster** ΓÇö Instead of manually structuring DITA XML, paste your content and generate valid Concept or Task topics instantly. This eliminates repetitive markup work and reduces formatting errors.

- **Draft and review in one place** ΓÇö The Initial Draft editor gives you a lightweight Word-like environment with formatting, comments, and replies. You can compose content, annotate it with review comments, and send it directly to the analysis engine without switching tools.

- **Collaborate through comments** ΓÇö The Word-style commenting system lets you select text, add annotations, and have threaded reply conversations. Comments persist across sessions so review context is never lost.
...
- Rule categories include:
  - **Grammar** ΓÇö double negatives, subjunctive mood, discontinuous phrasal verbs, missing "that"
  - **Word Usage** ΓÇö controlled vocabulary enforcement (e.g., "utilize" ΓåÆ "use", "login" ΓåÆ "sign in")
  - **Pronouns** ΓÇö vague pronoun detection, gendered language
  - **Style & Tone** ΓÇö politeness markers, biased/violent terminology, spatial references
  - **Structure** ΓÇö passive voice, sentence length, future tense
...
- Rich text editing with a familiar toolbar:
  ...
  - Heading styles (H1ΓÇôH4, Normal, Monospace)
...
### Python Dependencies
The backend uses **Python standard library only** ΓÇö no additional pip packages are required.
...
- **mammoth.js** ΓÇö DOCX to HTML conversion
- **docx.js** ΓÇö DOCX file generation (Save as Word)
- **FileSaver.js** ΓÇö Client-side file downloads
- **JSZip** ΓÇö ZIP file reading (for DITA map uploads)
- **SheetJS (xlsx)** ΓÇö Excel file parsing (for JIRA exports)
...
```
Γö£ΓöÇΓöÇ electron/
Γöé   Γö£ΓöÇΓöÇ main.js          # Electron main process, window management, IPC
Γöé   ΓööΓöÇΓöÇ preload.js       # Context bridge for secure renderer-to-main communication
Γö£ΓöÇΓöÇ frontend/
Γöé   Γö£ΓöÇΓöÇ index.html       # Application UI layout
Γöé   Γö£ΓöÇΓöÇ app.js           # Frontend logic (DOM, events, rendering)
Γöé   Γö£ΓöÇΓöÇ styles.css       # Application styles
Γöé   ΓööΓöÇΓöÇ lib/             # Bundled third-party libraries
Γö£ΓöÇΓöÇ backend/
Γöé   Γö£ΓöÇΓöÇ protocol.py      # JSON-over-stdin/stdout IPC protocol handler
Γöé   Γö£ΓöÇΓöÇ analyzer.py      # Core analysis orchestration
Γöé   Γö£ΓöÇΓöÇ rules.py         # Content analysis and rewrite rule definitions
Γöé   ΓööΓöÇΓöÇ utils.py         # Shared utilities (tokenization, XML generation)
...
- **Electron Shell** ΓÇö Creates the desktop window, spawns the Python backend, and routes IPC messages between frontend and backend
- **Frontend (HTML/CSS/JS)** ΓÇö Handles all UI rendering, user interaction, and display logic
- **Backend (Python)** ΓÇö Handles content analysis, rule evaluation, DITA conversion, and impact analysis logic
...
UNLICENSED ΓÇö Private application.
